...
class Clean(QWidget):
    ...
    def Gui(self):
        ...
        self.edit_button.setChecked(True)  # imposta lo stato predefinito del pulsante di modifica su False
        ...
        self.passbox1_button = QPushButton(QIcon('icons/pending.png'), '', self)
        ...
        if self.access == "cleanroom" or self.access == "guest":
    ...
    def update_table(self):
        self.start_timer(60)  # Aggiorna la tabella ogni minuto
        # Recupera i dati dalla tabella del foglio di lavoro Noclean
        data = wnc.get_all_values()[1:]  # esclude la prima riga (intestazioni delle colonne)
    ...
    def box_mode(self, process, process1):
        ...
        for row in selected_rows:
            if self.requests_table.item(row, 4).text() != process:
                QMessageBox.warning(self, 'Attenzione',
                                     'Non è possibile proseguire con la richiesta. Richieste multiple non compatibili"')
                return
        for i in range(self.requests_table.rowCount()):
            if self.requests_table.item(i, 0).checkState() == Qt.Checked:
                partnumber = self.requests_table.item(i, 1).text()
                part_number = wnc.col_values(1)
                ...
                for idx in indices:
                    if wnc.cell(row=idx, col=4).value == process:
                        wnc.update_cell(row=idx, col=4, value= process1)
                        wnc.update_cell(row=idx, col=5, value=datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
                        break
    ...
    def passbox_mode(self):
        self.box_mode('Pending', 'Passbox')

    def passbox1_mode(self):
        self.box_mode('Passbox', 'Pending')
